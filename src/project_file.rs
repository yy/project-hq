...
pub enum ProjectFileError {
    ...
    CheckboxConflict,
    WaitingOnRequired { file: String, people: Vec<String> },
...
impl fmt::Display for ProjectFileError {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            ...
            Self::WaitingOnRequired { file, people } => {
                if people.is_empty() {
                    write!(f, "waiting_on is required for {file}: no &person found")
                } else {
                    write!(
                        f,
                        "waiting_on is required for {file}: multiple people found ({})",
                        people.join(", ")
                    )
                }
            }
