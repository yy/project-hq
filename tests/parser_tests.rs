...
#[test]
fn move_project_updates_fields_with_space_before_colon() {
    let tmp = setup_dir();
    let base = tmp.path();
    write_project(
        base,
    ...
    assert_eq!(text.matches("priority").count(), 1);
...
#[test]
fn reorder_inserts_priority_after_status_with_space_before_colon() {
    let tmp = setup_dir();
    let base = tmp.path();
    write_project(
        base,
...
#[test]
fn reorder_is_atomic_when_any_file_is_invalid() {
    let tmp = setup_dir();
    let base = tmp.path();
    write_project(
        base,
